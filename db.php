...
class database {
	...
	function CreateTable ($dbname,$TableName) {
		$dbh = $this->mysqli;
		echo $TableName;
        $res = $dbh->query("CREATE TABLE $dbname.$TableName ( `ID` INT NOT NULL AUTO_INCREMENT , `name` VARCHAR(50) NOT NULL , `phone` VARCHAR(25) NOT NULL , PRIMARY KEY (`ID`), INDEX `name` (`name`)) ENGINE = MyISAM CHARSET=utf8 COLLATE utf8_general_ci;");
		var_dump($res);
	return ($res);

	}
	function addContact($tblname, $post) {
		$dbh = $this->mysqli;
			//echo $tblname."<br>";	
			//print_r ($post);	
	...
	function delContact($tblname, $id) {
		$dbh = $this->mysqli;
